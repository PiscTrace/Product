...
| 类别         | 项目                           | 描述说明                                         |
|--------------|----------------------------------|--------------------------------------------------|
| 支持版本     | v8 / v9 / v10 / v11 / v12       | 全系列 YOLO 模型支持                             |
...
## 为什么没有 MiDaS 选项
- **MiDaS** 使用 GitHub 在线资源和在线配置。
- 使用时请确保您的计算机可以访问 **GitHub 网络**。
- 模型首次使用加载时间较长，加载配置时间较长。
...
## 摄像头不支持渲染保存
- 受硬件环境、可分配计算资源和计算复杂程度影响，摄像视频流只支持实时预览不支持录制运算结果
- 支持同一多开，可预览同时录制原视频，保存在 `项目地址/input`内

## 如何切换摄像头/文件
- 在文件 > 切换数据源
- 设计上没有限制摄像头数量，需要考虑实际硬件资源适当接入

## 自己微调的YOLO模型
- 模型放置在模型文件夹下
- 替换`解压地址/_internal/ultralytics`等相关库文件，注意不要改动加载模型，视图帧输入和推理结果输出相关数据接口
